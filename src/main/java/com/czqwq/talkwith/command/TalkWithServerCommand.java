    ...
    public void processCommand(ICommandSender sender, String[] args) throws CommandException {
        ...
        switch (args[0].toLowerCase()) {
            case "share" -> {
                ...
                SharedSession session = new SharedSession(player.getUniqueID(), player.getCommandSenderName(),
                ...
                PacketHandler.INSTANCE.sendTo(new PacketShareInvite(player.getCommandSenderName(), session.sessionId), target);
            }
            case "mute" -> {
                if (args.length < 2) { player.addChatMessage(new ChatComponentText("§c[TalkWith]§r Usage: /talkwith mute <playerName>")); return; }
                SharedSession session = getOwnedSession(player);
                if (session == null) { player.addChatMessage(new ChatComponentText("§c[TalkWith]§r You don't own a session.")); return; }
                EntityPlayerMP target = MinecraftServer.getServer().getConfigurationManager().func_152612_a(args[1]);
                if (target != null) {
            ...
            case "unmute" -> {
                if (args.length < 2) { player.addChatMessage(new ChatComponentText("§c[TalkWith]§r Usage: /talkwith unmute <playerName>")); return; }
                SharedSession session = getOwnedSession(player);
                if (session == null) { player.addChatMessage(new ChatComponentText("§c[TalkWith]§r You don't own a session.")); return; }
                EntityPlayerMP target = MinecraftServer.getServer().getConfigurationManager().func_152612_a(args[1]);
                if (target != null) {
            ...
            case "kick" -> {
                if (args.length < 2) { player.addChatMessage(new ChatComponentText("§c[TalkWith]§r Usage: /talkwith kick <playerName>")); return; }
                SharedSession session = getOwnedSession(player);
                if (session == null) { player.addChatMessage(new ChatComponentText("§c[TalkWith]§r You don't own a session.")); return; }
                EntityPlayerMP target = MinecraftServer.getServer().getConfigurationManager().func_152612_a(args[1]);
                if (target != null) {
            ...
            case "cooldown" -> {
                if (args.length < 2) { player.addChatMessage(new ChatComponentText("§c[TalkWith]§r Usage: /talkwith cooldown <seconds>")); return; }
                SharedSession session = getOwnedSession(player);
                if (session == null) { player.addChatMessage(new ChatComponentText("§c[TalkWith]§r You don't own a session.")); return; }
                try {
                    session.cooldown = Integer.parseInt(args[1]);
                    player.addChatMessage(new ChatComponentText("§a[TalkWith]§r Cooldown set to " + session.cooldown + "s."));
            ...
            case "close" -> {
                ...
                for (UUID uuid : session.players) {
                    EntityPlayerMP member = MinecraftServer.getServer().getConfigurationManager().getPlayerByUUID(uuid);
                    if (member != null) {
