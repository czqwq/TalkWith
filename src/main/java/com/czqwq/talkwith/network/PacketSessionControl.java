        ...
        public IMessage onMessage(PacketSessionControl msg, MessageContext ctx) {
            ...
            switch (msg.action) {
                case "single" -> {
                    ...
                    player.addChatMessage(new ChatComponentText("§a[TalkWith]§r Left shared session. Back to single mode."));
                }
                case "mute" -> {
                    if (!session.ownerUuid.equals(playerUuid)) {
                        player.addChatMessage(new ChatComponentText("§c[TalkWith]§r Only the session owner can do that."));
                        return null;
                    }
                    EntityPlayerMP target = server.getConfigurationManager().func_152612_a(msg.target);
                    if (target != null) {
                ...
                case "close" -> {
                    if (!session.ownerUuid.equals(playerUuid)) {
                        player.addChatMessage(new ChatComponentText("§c[TalkWith]§r Only the session owner can close the session."));
                        return null;
                    }
                    SharedSession.sessions.remove(session.sessionId);
